...
from train.oln_tfm_enc_dec_spk_pit_on_the_fly import SpeakerDiarization
...
def train(configs, gpus, checkpoint_resume, test_folder=None):
    ...
    # ckpts = [x for x in all_files if (".ckpt" in x) and ("epoch" in x) and int(x.split("=")[1].split("-")[0])>=start_epo and int(x.split("=")[1].split("-")[0])<=end_epo]
    # ckpts = [x for x in all_files if (".ckpt" in x) and ("epoch" in x) and int(x.split("=")[2].split(".")[0])>=configs["log"]["start_step"] and int(x.split("=")[2].split(".")[0])<=configs["log"]["end_step"]]
    # ckpts = [x for x in all_files if (".ckpt" in x) and ("epoch" in x) and int(x.split("=")[1].split("-")[0]) in configs["log"]["epochs"]]
