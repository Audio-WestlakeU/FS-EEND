...
class SpeakerDiarization(pl.LightningModule):
    ...
    def test_step(self, batch, batch_index):
        ...
        # # feats = torch.rand(seq_len, 345)
        # feats = [feats]
        # clip_lengths = [seq_len]
        # st_time = time.time()
        num_spks = "4"
        version = "_tfm_10w_ver_204"
        save_dir_parnt = f"./data/onl_{num_spks}spk_version{version}"
        cos_oup_dir = os.path.join(save_dir_parnt, "cos_map")
        if not os.path.isdir(cos_oup_dir):
                os.makedirs(cos_oup_dir)
        cos_oup_file = cos_oup_dir + "/cos_map_ck.txt"
        st_time = time.time()
        ...
        print(rtf, rec, clip_lengths[0]/10)
        labels_realspk = [l[:, 1:-1] for l in labels]
        stats = report_diarization_error(preds_realspk, labels_realspk, label_delay=self.label_delay)
        ...
        # rtf_file = os.path.join(save_dir_parnt, self.dir_name) + f"/RTF_{int(seq_len/10)}s.txt"
        # with open(rtf_file, 'a') as f:
        #     f.write(str(seq_len))
        #     f.write("\t")
        #     f.write(str(rtf))
        #     f.write("\n")
                
        # print(attractors[0].norm(dim=-1))
        # print(embs[0].norm(dim=-1))
