...
class SpeakerDiarization(pl.LightningModule):
    ...
    def __init__(self, hparams, model, datasets, opt, scheduler, collate_func) -> None:
        ...
        self.seed = self.hparams["training"]["seed"]
        # self.chunk_size = chunk_size
        # self.der_output_file = der_output_file
    ...
    def training_step(self, batch, batch_index):
        ...
        preds_spks = [p[:, 1:n_spk+1] for p, n_spk in zip(preds, n_spks)]
        ...
        # pad with max_spks in training dataset
        # perm_labels = pad_labels(perm_labels, max_nspk + 2)
        # for pred in preds:
        #     if pred.shape[1] < max_nspk + 2:
        #         print(pred.shape[1], max_nspk + 2)
        # preds = pad_preds(preds, max_nspk + 2)
    ...
    def test_step(self, batch, batch_index):
        ...
        # feats_400 = [feats[0][:400]]
        # clip_lengths_400 = [400]
        # preds_400, embs, attractors = self.model.test(feats, clip_lengths, self.max_spks + 2)
        # if (not (abs(preds_400[0][:390] - preds[0][:390])<1e-6).all()):
        #     print(preds_400[0][:390])
        #     print(preds[0][:390])
        ...
        num_spks = "all"
        version = "_10w_ver_204_ch_ver_88"
    ...
    def construct_dataloader(self, dataset, seed, batch_size, shuffle, num_workers, collate_fn):
        return DataLoader(
            dataset,
            sampler=MyDistributedSampler(dataset, seed=seed, shuffle=shuffle),
