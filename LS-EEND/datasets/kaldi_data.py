...
def load_mel_scp(mel_scp_file):
    """ return dictionary { rec: mel_rxfilename } """
    lines = [line.strip().split(None, 1) for line in open(mel_scp_file)]
    return {x[0]: x[1] for x in lines}

def load_emb_scp(emb_scp_file):
    """ return dictionary { rec: wav_rxfilename } """
    lines = [line.strip().split(None, 1) for line in open(emb_scp_file)]
    return {x[0]: x[1] for x in lines}
...
@lru_cache(maxsize=1)
def load_emb(emb_rxfilename, start=0, end=None):
    data = np.load(emb_rxfilename)
    return data[start:end]

@lru_cache(maxsize=1)
def load_mel(mel_rxfilename, start=0, end=None):
    data = np.load(mel_rxfilename)
    return data[start:end]
...
def load_reco2num_spk(reco2num_spk_file):
    """ returns dictionary { recid: num_spk} """
    if not os.path.exists(reco2num_spk_file):
        return None
    lines = [line.strip().split(None, 1) for line in open(reco2num_spk_file)]
    return {x[0]: x[1] for x in lines}
...
def load_utt2timestamp(utt2timestamp_file):
    """ returns dictionary { uttid: timestamp }  """
    ret = {}
    if not os.path.exists(utt2timestamp_file):
        return None
    for line in open(utt2timestamp_file):
        utt, st, et = line.strip().split()
        if utt not in ret:
            ret[utt] = [st, et]
    return ret
...
class KaldiData:
    def __init__(self, data_dir):
        ...
        self.reco2num_spk = load_reco2num_spk(
                os.path.join(self.data_dir, "reco2num_spk"))
        # self.mels = load_mel_scp(
        #         os.path.join(self.data_dir,'mel.scp'))
        # self.embs = load_emb_scp(
        #         os.path.join(self.data_dir, 'emb.scp'))
        self.utt2timestamp = load_utt2timestamp(
            os.path.join(self.data_dir, "utt2timestamp"))
    ...
    def load_emb(self, recid, start=0, end=None):
        emb = load_emb(
            self.embs[recid], start, end)
        return emb
    
    # def load_mel(self, recid, start=0, end=None):
    #     mel = load_mel(
    #         self.mels[recid], start, end)
    #     return mel
