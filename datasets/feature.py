...
def get_ET(
        kaldi_obj,
        rec, start, end, frame_size, frame_shift, subsampling, rate,
        n_speakers=None,
        use_speaker_id=False):
    E = kaldi_obj.load_emb(rec, int(start / subsampling), int(end / subsampling))
    filtered_segments = kaldi_obj.segments[rec]
    # filtered_segments = kaldi_obj.segments[kaldi_obj.segments['rec'] == rec]
    speakers = np.unique(
            [kaldi_obj.utt2spk[seg['utt']] for seg
    ...
    if n_speakers is None:
        n_speakers = len(speakers)
    T = np.zeros((E.shape[0] * subsampling, n_speakers), dtype=np.int32)

    if use_speaker_id:
        all_speakers = sorted(kaldi_obj.spk2utt.keys())
        S = np.zeros((E.shape[0] * subsampling, len(all_speakers)), dtype=np.int32)
